...
//			'timeout' : 3000,
		$.getJSON('inputExecuteAction.php?id=1&method=Playlist.GetPlaylists',function(data) {
				if (data && data.result && data.result.length > 0) {
					$.each($(data.result), jQuery.proxy(function(i, item) {
						that.playlists[item.type] = item.playlistid;
					}, that));
				}
		});
	},
	remoteControlOpen : function(event) {
		...
		if (!libraryContainer || libraryContainer.length == 0) {
			...
			for ( var akey in keys) {
				...
				aremotekey.addClass('remote_key').css('height', keys[akey]['height']).css('width', keys[akey]['width'])
						.css('top', keys[akey]['top']).css('left', keys[akey]['left']).bind('click', {
							key : keys[akey]['name']
	...
	pressRemoteKey : function(event) {
		var player = -1, keyPressed = event.data.key;
		$('#spinner').show();
		switch (keyPressed) {
		case 'up':
	...
	generateThumb : function(type, thumbnail, title, artist) {
		...
		case 'album':
			...
			code = '<p class="album" title="' + title + '">' + title + '</p><p class="artist" title="' + artist + '">'
					+ artist + '</p>';
			break;
		case 'movie':
		...
		return floatableAlbum.addClass(className).html(
				'<div class="imgWrapper"><div class="inner"><img src="' + path + '" alt="' + title + '" /></div></div>'
						+ code);
	},
	showAlbumSelectorBlock : function(album) {
		if (album) {
			...
			if (!albumSelectorBlock || albumSelectorBlock.length == 0) {
				...
				albumSelectorBlock
						.attr('id', 'albumSelector')
						.html(
								'<table><tr><td class="allAlbums">All Albums</td><td class="activeAlbumTitle"></td><td class="prevAlbum">&nbsp;</td><td class="nextAlbum">&nbsp;</td></tr></table>');
...
xbmc.rpc.request({
						...
						'success' : function(data) {
							...
							albumDetailsContainer
									.attr('id', 'albumDetails' + event.data.album.albumid)
									.addClass('contentContainer')
									.addClass('albumContainer')
									.html(
											'<table class="albumView"><thead><tr class="headerRow"><th>Artwork</th><th>&nbsp;</th><th>Name</th><th class="time">Time</th><th>Artist</th><th>Genre</th></tr></thead><tbody class="resultSet"></tbody></table>');
							$('.contentContainer').hide();
							...
							$('#albumDetails' + event.data.album.albumid + ' .albumThumb').append(
									this.generateThumb('album', albumThumbnail, albumTitle, albumArtist)).append(
									$('<div>').addClass('footerPadding'));
							$('#spinner').hide();
	...
	togglePosterView : function(event) {
		...
		$(
				".floatableTVShowCover, .floatableTVShowCover div.imgWrapper, .floatableTVShowCover img, .floatableTVShowCover div.imgWrapper div.inner")
				.css('width', wthumblist).css('height', hthumblist);
		$(
				".floatableTVShowCoverSeason div.imgWrapper, .floatableTVShowCoverSeason div.imgWrapper div.inner,.floatableTVShowCoverSeason img, .floatableTVShowCoverSeason")
...
xbmc.rpc.request({
				...
				'params' : {
					'properties' : [ 'season', 'showtitle', 'playcount', 'episode', 'thumbnail', 'fanart' ],
					'tvshowid' : event.data.tvshow.tvshowid
				},
				'success' : function(data) {
					tvshowDetailsContainer = $('<div>');
					tvshowDetailsContainer.attr('id', 'tvShowDetails' + event.data.tvshow.tvshowid).css('display',
							'none').addClass('contentContainer').addClass('tvshowContainer');
...
xbmc.rpc.request({
					...
					'success' : function(data) {
						...
						var episodeTable = $('<table>')
								.addClass('seasonView')
								.html(
										'<thead><tr class="headerRow"><th class="thumbHeader">N&deg;</th><th>Title</th><th class="thumbHeader">Thumb</th><th class="thumbHeader">Details</th></tr></thead><tbody class="resultSet"></tbody>');
						$.each($(data.result.episodes), jQuery.proxy(function(i, item) {
							...
							var episodePicture = $('<td>').addClass('episodeThumb').append(episodePictureImg).bind(
									'click', {
										episode : item
							...
							var episodeDetails = $('<td class="info">').html('').bind('click', {
								episode : item
							}, jQuery.proxy(this.displayEpisodeDetails, this)).css('cursor', 'pointer');
							episodeRow.append(episodeNumber).append(episodeTitle).append(episodePicture).append(
									episodeDetails);
	...
	displayEpisodeDetails : function(event) {
		...
		episodeDetails.append($('<img>').attr('src', 'images/close-button.png').addClass('closeButton').bind('click',
				jQuery.proxy(this.hideOverlay, this)));
		episodeDetails.append($('<img>').attr('src', this.getThumbnailPath(event.data.episode.thumbnail)).addClass(
				'episodeCover'));
		...
		if (event.data.episode.runtime) {
			episodeDetails.append($('<p>').addClass('runtime').html(
					'<strong>Runtime:</strong> ' + event.data.episode.runtime + ' minutes'));
		}
		if (event.data.episode.season) {
			episodeDetails.append($('<p>').addClass('season').html(
					'<strong>Season:</strong> ' + event.data.episode.season));
		}
		if (event.data.episode.episode) {
			episodeDetails.append($('<p>').addClass('episode').html(
					'<strong>Episode:</strong> ' + event.data.episode.episode));
		}
		if (event.data.episode.plot) {
			episodeDetails.append($('<p>').addClass('plot').html(
					'<strong>Plot:</strong> <br/><br/>' + event.data.episode.plot));
		}
		if (event.data.episode.genre) {
			episodeDetails.append($('<p>').addClass('genre')
					.html('<strong>Genre:</strong> ' + event.data.episode.genre));
		}
		if (event.data.episode.director) {
			episodeDetails.append($('<p>').addClass('director').html(
					'<strong>Directed By:</strong> ' + event.data.episode.director));
		}
	...
	displayMovieDetails : function(event) {
		...
		movieDetails.append($('<img>').attr('src', 'images/close-button.png').addClass('closeButton').bind('click',
				jQuery.proxy(this.hideOverlay, this)));
		...
		if (event.data.movie.runtime) {
			movieDetails.append($('<p>').addClass('runtime').html(
					'<strong>Runtime:</strong> ' + event.data.movie.runtime + ' minutes'));
		}
		...
		if (event.data.movie.director) {
			movieDetails.append($('<p>').addClass('director').html(
					'<strong>Directed By:</strong> ' + event.data.movie.director));
		}
...
xbmc.rpc.request({
				...
				'params': {
					...
					'properties' : [ 'genre', 'director', 'trailer', 'tagline', 'plot', 'plotoutline', 'title',
							'originaltitle', 'lastplayed', 'runtime', 'year', 'playcount', 'rating', 'thumbnail',
							'file' ],
					'sort' : {
						'method' : 'sorttitle',
						'ignorearticle' : true
	...
	tvshowLibraryOpen : function() {
		...
		if (!libraryContainer || libraryContainer.length == 0) {
			...
			togglePoster.attr('id', 'togglePoster').css('cursor', 'pointer').bind('click', {
				mode : 'poster'
			}, jQuery.proxy(this.togglePosterView, this));
			toggleBanner = $('<span>Banner</span>');
			toggleBanner.attr('id', 'toggleBanner').css('cursor', 'pointer').addClass('activeMode').bind('click', {
				mode : 'banner'
			}, jQuery.proxy(this.togglePosterView, this));
			toggleLandscape = $('<span>Landscape</span>');
			toggleLandscape.attr('id', 'toggleLandscape').css('cursor', 'pointer').bind('click', {
				mode : 'landscape'
			}, jQuery.proxy(this.togglePosterView, this));
...
xbmc.rpc.request({
				...
				'params' : {
					'properties' : [ 'genre', 'plot', 'title', 'lastplayed', 'episode', 'year', 'playcount', 'rating',
							'thumbnail', 'studio', 'mpaa', 'premiered' ]
				},
				'success' : function(data) {
					if (data && data.result && data.result.tvshows) {
						libraryContainer = $('<div>');
						libraryContainer.append(toggle);
						libraryContainer.attr('id', 'tvshowLibraryContainer').addClass('contentContainer');
						$('#content').append(libraryContainer);
					} else {
						libraryContainer.html('');
					}
...
xbmc.rpc.request({
				...
				'success' : function(data) {
					if (data && data.result && (data.result.directories || data.result.files)) {
						libraryContainer = $('<div>');
						libraryContainer.attr('id', 'pictureLibraryDirContainer' + directory).addClass(
								'contentContainer');
						$('#content').append(libraryContainer);
						var breadcrumb = $('<div>');
						var seperator = '/';
						...
						jQuery.each(directoryArray, function(i, v) {
							if (v != '') {
								item += v + seperator;
								breadcrumb.append($('<div>').text(' > ' + v).css('float', 'left')
										.addClass('breadcrumb'));
							}
						});
			...
			xbmc.rpc.request({
				...
				'success' : function(data) {
					if (data && data.result && data.result.shares) {
						libraryContainer = $('<div>');
						libraryContainer.attr('id', 'pictureLibraryContainer').addClass('contentContainer');
						$('#content').append(libraryContainer);
					} else {
						libraryContainer.html('');
					}
