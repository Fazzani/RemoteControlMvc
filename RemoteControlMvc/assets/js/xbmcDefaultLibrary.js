$(function () {
    ...
    (function (window) {
        ...
        xbmc.core = {
            ...
            'durationToString': function (duration) {
                var total_seconds = duration || 0, seconds = total_seconds % 60, minutes = Math.floor(total_seconds / 60) % 60, hours = Math.floor(total_seconds / 3600), result = ((hours > 0 && ((hours < 10 ? '0' : '') + hours + ':')) || '');
    ...
    NowPlayingManager.prototype = {
        ...
        updateState: function () {
            ...
            $.getJSON('inputExecuteAction.php?id=1&method=Player.GetActivePlayers',
                    function (data) {
                    ...
                    + that.activePlayerId + ',"properties":["playlistid","speed","position","totaltime","time"]}',
                            function (data) {
                                ...
                                if ((that.autoRefreshAudioData || that.autoRefreshVideoData) && !that.activeItemTimer) {
                                    ...
                                    //setTimeout(jQuery.proxy(that.updateActiveItemDurationLoop, that), 1000);
                                }
        ...
        nextTrack: function () {
            if (that.activePlayer) {
            	$.getJSON('inputExecuteAction.php?id=1&method=Player.GoTo&param={"playerid":' + that.activePlayerId + ',"to":"next"}', function (data) { });
            }
        },
        prevTrack: function () {
            if (that.activePlayer) {
                $.getJSON('inputExecuteAction.php?id=1&method=Player.GoTo&param={"playerid":' + that.activePlayerId + ',"to":"previous"}', function (data) { });
            }
        },
        stopTrack: function () {
            if (that.activePlayer) {
                $.getJSON('inputExecuteAction.php?id=1&method=Player.Stop&param={"playerid":' + that.activePlayerId + '}', function (data) { });
            }
        },
        playPauseTrack: function () {
            if (that.activePlayer) {
                var method = ((that.playing || that.paused)
                        ? 'Player.PlayPause'
                        : 'Playlist.Play');
                $.getJSON('inputExecuteAction.php?id=1&method=' + method + '&param={"playerid":' + that.activePlayerId + '}', function (data) { });
            }
        },
        ...
        playPlaylistItem: function (sender) {
            ...
            if (!that.activePlaylistItem || (this.activePlaylistItem !== undefined && sequenceId != that.activePlaylistItem.seq)) {
                xbmc.rpc.request({
                    'method': 'Player.GoTo',
                    'params': {
            ...
            $.getJSON('inputExecuteAction.php?id=1&method=Playlist.GetItems&param={"playlistid":'
                            ...
                            function (data) {
                                if (data && data.result && data.result.items && data.result.items.length > 0 && data.result.limits.total > 0) {
                                    if (!that.activePlaylistItem || that.playlistChanged(data.result.items) || (that.activePlaylistItem && (that.activePlaylistItem.seq != that.currentItem))) {
                                        var ul = $('<ul>');
                                        var activeItem;
                                        $.each($(data.result.items), jQuery.proxy(
                                                function (i, item) {
                                                    var li = $('<li>');
        ...
        refreshAudioData: function () {
            ...
            if (that.activePlaylistItem) {
                if (that.activePlaylistItem != that.lastPlaylistItem) {
                    ...
                    $('#audioCoverArt').html('<img src="' + imgPath + '" alt="' + that.activePlaylistItem.album + ' cover art">');
                    $('#audioTrackTitle').html('<span title="' + that.activePlaylistItem.title + '">' + that.activePlaylistItem.title + '</span>');
                    if (that.activePlaylistItem.album) {
                        $('#audioAlbumTitle').html('<span title="' + that.activePlaylistItem.album + '">' + that.activePlaylistItem.album + '</span>').show();
                    } else {
                ...
                $('#audioDuration').html(xbmc.core.durationToString(that.trackBaseTime) + ' / ' + xbmc.core.durationToString(that.trackDurationTime));
        ...
        refreshVideoData: function () {
            ...
            if (that.activePlaylistItem) {
                if (JSON.stringify(that.activePlaylistItem) != JSON.stringify(that.lastPlaylistItem)) {
                	...
                	console.log('true is diff that.activePlaylistItem :'  + JSON.stringify(that.activePlaylistItem) +'      that.lastPlaylistItem : '+ JSON.stringify(that.lastPlaylistItem));
                	that.lastPlaylistItem = that.activePlaylistItem;
                    ...
                    $('#videoCoverArt').html('<img src="' + imgPath + '" alt="' + that.activePlaylistItem.title + ' cover art">');
                    ...
                    if(that.ischannel)
                    	$('#videoTitle').html(extra + '<span class="channelnumber">'+that.activePlaylistItem.channelnumber+'- </span>'+that.activePlaylistItem.label);
                    else
                    	$('#videoTitle').html(extra + that.activePlaylistItem.title);
                }
                if(!that.ischannel){
                	$('#videoDuration').html(xbmc.core.durationToString(that.trackBaseTime) + ' / ' + xbmc.core.durationToString(that.trackDurationTime));
                	var buttonWidth = $('#progressBar .progressIndicator').width();
                	var progressBarWidth = (that.trackBaseTime / that.trackDurationTime)  * 100;
                	var progressSliderPosition = Math.ceil(($('#progressBar').width() / 100) * progressBarWidth) - buttonWidth;
                	if (progressSliderPosition < 0) {
            ...
            $.getJSON('inputExecuteAction.php?id=1&method=Playlist.GetItems&param={"playlistid":2' + ''
                            ...
                            function (data) {
                                if (data && data.result && data.result.items
                                    ...
                                    $.getJSON('inputExecuteAction.php?id=1&method=Player.GetItem&param={"playerid":' + that.activePlayerId
                                ...
                                if (that.autoRefreshVideoPlaylist) {
                                    //setTimeout(jQuery.proxy(that.updateVideoPlaylist, that), 1000);
                                }
                            });
        }
    }
    ...
    var notConnectedEvent = jQuery.Event("connectionLoosed");
     ...
     setInterval(function () {
    	 ...
    	 { 
    		 ...
    		 $.getJSON('inputExecuteAction.php?method=JSONRPC.Ping'
    		 ).done(function (data) {
